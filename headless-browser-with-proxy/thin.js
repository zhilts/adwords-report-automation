...
Mitm.prototype.direct = function (req, res) {
    ...
    var captureInterceptor = this.opts.captureInterceptor;
    ...
    req.on('end', function () {
        ...
        d.on('error', function(err){
            console.error("proxy request domain:", err.toString());
        });

        d.run(function(){
            params.body = buffer;
            var r = request(params, function (err) {
                if (err) {
                    self.emit('error', err);
                    res.end();
                }
